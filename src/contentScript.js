  ...
  switch (button) {
    ...
    case "{enter}":
      if (inputElement.tagName.toLowerCase() === "textarea") {
        ...
        if (pos !== null) {
          inputElement.value =
            inputElement.value.substr(0, pos) +
            button +
            inputElement.value.substr(posEnd);
    ...
    case "{bksp}":
      ...
      inputElement.value =
        String(inputElement.value).substr(0, pos) +
        String(inputElement.value).substr(posEnd);
...
function performNativeKeyPress(element, keyCode) {
  ...
  element.dispatchEvent(
    new Event("keypress", { keyCode: keyCode, which: keyCode })
  );
